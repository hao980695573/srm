<template>
  <div class='yulan_warp' v-show='pages>0'>
    <div id="samllPrinting">
      <div v-for="(page,index) in pages" :key="index" :class='[index%2==0?"":["a","margintow"]]'>
        <div :class='[index%2==0?"":["a","tab_company_out"]]'>
          <!--表格内容-->
    ...
    <!--打印按钮-->
    <div class='printBth'>
      <el-button type="primary" size='mini' @click='smallprintPdf'>打印</el-button>
    </div>
  </div>
      ...
      smallprintPdf() {
        ...
        axios.post(config.api, '/barcode/updateBarcodeStatus.do', params).then((res) => {
          ...
          if (res.status == 0) {
            ...
            setTimeout(function(){
              LODOP=getLodop();
              var strStyleCSS2="<link href='../../../../static/styles/smallBarPrint6.css' type='text/css' rel='stylesheet'>";
              var strFormHtml="<head>"+strStyleCSS2+"</head><body>"+document.getElementById("samllPrinting").innerHTML+"</body>";

              LODOP.ADD_PRINT_HTM("1mm","6mm","RightMargin:5cm","1100", strFormHtml);

  ...
  <el-dialog title="新增信息" :visible.sync="addShow" width='1300px' top='40px' @close='clearData'>
    <el-form ref="screen" :model="screen_form" label-width="80px" size='mini' class='screen_form'>
      <el-form-item label="版本">
        <el-input v-model="screen_form.version" placeholder="请输入标题"></el-input>
      </el-form-item>
    </el-form>
    <!--内容-->
    <div class='center'>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      publish_buttom() {
        const params = {
          version:this.screen_form.version,
          updateContent: this.$refs.editor.getContent()
        }
        axios.post(config.api, '/renewalProcess/addRenewalProcess.do', params).then((res) => {
          if (res.status == 0) {
            let _this = this
            this.$alert(res.msg, {

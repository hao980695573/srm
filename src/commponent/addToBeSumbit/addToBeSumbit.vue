    ...
    <div class='inquiry_warp'>
      <el-form ref="screen_data" :model="screen_data" :inline="true" size='mini'>
        ...
        <div>
          ...
          <el-button type="success" class='Fuzzy' size="mini" @click='financeAccount("pass")' :disabled='this.deliveryProductList==""'>提交</el-button>
          <el-button type="warning" class='Fuzzy' size="mini" @click='financeAccount("examine")' :disabled='this.deliveryProductList==""'>撤回</el-button>
        </div>
    ...
    >
      ...
      <div style='margin-top:20px'>
        <el-form ref="detailList" :model="detailList" size='mini' label-width="80px" :inline="true">
          ...
          <el-form-item label="对账退回说明" label-width='100px'>
  ...
  export default {
    ...
    methods:{
      ...
      handleCurrentChange(data){
        ...
        axios.post(config.api, '/aStatement/getToBeSubmitList.do', params).then((res) => {
          console.log(res);
          if (res.status == 0) {
            this.form=res.data
            // 分页
      ...
      financeAccount(data){
        if(data=='pass'){
          // 后台要求增加状态字段
          this.deliveryProductList.status=1
        }else if(data=='examine'){
          this.deliveryProductList.status=0
        }

        console.log(this.deliveryProductList);

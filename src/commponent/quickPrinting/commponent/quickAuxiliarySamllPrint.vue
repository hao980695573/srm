<template>
  <div class='yulan_warp'>
    ...
    <!--打印按钮-->
    <div class='printBth'>
      <el-button type="primary" size='mini' @click='auxiliaryMiddlebth'>打印</el-button>
    </div>
  </div>
        ...
        axios.post(config.api, '/barcode/updateBarcodeStatus.do', params).then((res) => {
          if (res.status == 0) {
            ...
            setTimeout(function () {
              ...
              var strFormHtml = "<head>" + strStyleCSS2 + "</head><body>" + document.getElementById("auxiliarySamll").innerHTML + "</body>";

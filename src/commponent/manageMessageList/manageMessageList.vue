  ...
  <div class='list_warp'>
    ...
    >
      ...
      <el-table-column
        ...
        width="100">
        <template slot-scope="scope">
          <span>{{scope.row.messageSender}}</span>
        </template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods:{
      // 获取 uuid 角色身份
      ...mapMutations(['changeMessageDetail','changeDrawerShow','changeDrawerEdit']),
      // 获取列表
      getList(data){
        if(data=='screen'){
          this.pageNum=1
        ...
        axios.post(config.api, '/message/manageMessageList.do', params).then((res) => {
          // 赋值档案列表
          if (res.status == 0) {
            this.List=res.data

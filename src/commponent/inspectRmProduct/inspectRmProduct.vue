    ...
    methods: {
      ...
      saveBth() {
        ...
        if(this.submitState){
          ...
          // 校验通过
          if(this.CountState){
            // 后台传入的数据和要发过去的字段不一样，重新生成一个对象
            let Arr=[]
            this.tableData.map((item)=>{
              Arr.push({
                // 补货日期
                purchaseDate:item.createDate,
                // 补货单号
                bilNo:item.purchaseOrder,
                // 品号
                prdNo:item.productNumber,
                // 品名
                ...
                freeId:false,
                // 货品特征
                prdMark:item.characteristics,
                // 项次
                itm:item.item,
                //库位
                wh:item.wh,
                // 送货单号
                shNoCus:item.rmNumber,
                // 产品订单
                osNo:item.purchaseOrder,
            ...
            const params = {
              // 到货日期
              tiDd:new Date(this.arrivalDate+" "+"00:00:00"),
            ...
            console.log(params);
            axios.post(config.api, '/replenishment/inspectRmProduct.do', params).then((res) => {
              // 允许提交
              ...
              if (res.status == 0) {
                ...
                _this.$alert('保存成功', {
                  ...
                  callback: action => {
                    ...
                    this.tableData=[]
                    // 回显送检单号
                    this.entryEumber=res.data.tiNo
                  }
                })
              }else{
                this.$alert(res.msg, {

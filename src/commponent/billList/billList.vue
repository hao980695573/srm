<template>
  <div>
      ...
      <el-form ref="screen_data" :model="screen_data" :inline="true" size='mini'>
        ...
        <div>
          ...
          <el-form-item label="单据状态">
            <el-select v-model="screen_data.billStatus" placeholder="单据状态">
              <el-option label="请选择" value="99"></el-option>
              <el-option label="供应商已发起" value="1"></el-option>
              <el-option label="财务审核通过" value="2"></el-option>
              <el-option label="财务审核不通过" value="3"></el-option>
              <el-option label="已保存发票" value="4"></el-option>
              <el-option label="终审通过" value="5"></el-option>
    ...
    >
      ...
      <el-table-column label="操作" width="180" align="center">
        <template slot-scope="scope">
          ...
          <el-button type="text" size='mini' @click='InvoiceShow(scope.row)'>发票</el-button>
          <el-button type="text" size="small" @click='enterPrint(scope.row)'>打印</el-button>
          <el-button type="text" size="small" @click='downloadFiles(scope)'>下载</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页-->
    ...
    >
      <!--搜索条件-->
      <div>
        <el-form ref="invoice" :model="invoice" size='mini' label-width="80px" :inline="true">
          <el-form-item label="发票号">
            <el-input v-model="invoice.invoiceObject" style='width: 230px' placeholder="请输入发票号"></el-input>
          </el-form-item>
          <!--保存-->
          <el-button type="primary" size='mini' style='margin-left:20px' @click='InvoiceNumber'>保存</el-button>
        </el-form>
      </div>
    </el-dialog>
    <!--查看详情-->
      ...
      <div>
        <el-form ref="detailedScreen" :model="detailedScreen" size='mini' label-width="80px" :inline="true">
          ...
          <el-form-item>
            ...
            <el-button type="danger" @click='deleteDetails' :disabled='detailedScreen.search==""' v-if="userRole!='FIRMMANAGER'&&userRole!='FIRMUSER'&&userRole!='FIRMCLERK'">删除</el-button>
    ...
    data() {
      return {
        ...
        rules: {
          // 物料单号
          deliveryNumber: [
            { required: false, message: '物料编码不能为空', trigger: 'blur' }
          ],
          // 采购单号
          purchaseOrder:[
    ...
    methods: {
      ...
      handleCurrentChange(data) {
        ...
        const params = {
          ...
          billStatus: this.screen_data.billStatus
        }
        axios.post(config.api, '/aStatement/getBillList.do', params).then((res) => {
          if (res.status == 0) {
            this.form = res.data
            // 分页
      ...
      // 显示编辑发票
      InvoiceShow(scope){
        this.detailsInvoice=scope
        this.invoiceDialog=true
      },
      // 填入发票
      InvoiceNumber(){
          this.detailsInvoice.invoiceNumber=this.invoice.invoiceObject
          axios.post(config.api, '/aStatement/SaveInvoice.do', this.detailsInvoice).then((res) => {
            if (res.status == 0) {
              this.$alert(res.msg, {
      ...
      DetailListSelection(selection, row){
        ...
        // 只有退款的可以选
        if(row.dataType!=2){
          this.$refs.SeeDetailList.toggleRowSelection(row,false);
        }else{
          // 选中的复制

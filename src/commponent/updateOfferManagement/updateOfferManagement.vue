      ...
      <div class='inquiry_warp'>
        <el-form ref="base" :model="form" :rules="rules" label-width="100px">
          <el-form-item label="询报价类型" prop="tenderType">
            <el-select v-model="form.tenderType" placeholder="请选择询报价类型" @change="currentSel" disabled>
            ...
            <el-form size="small" ref="manufacturerList" :model="form">
              <el-row v-for="(item,index) in form.manufacturerList" :key="index">
                <el-col :span="8">
                  ...
                  >
                    <el-select v-model="item.manufacturerName" placeholder="请选择厂商" style='width: 100%' @change="((val)=>{changeFacturerList(val, index)})" disabled>
                      <el-option :label="item.companyName" :value="item.companyName" v-for='(item,index) in options' :key='index'></el-option>
                    </el-select>
                  </el-form-item>
                </el-col>
                <el-col :span="8" :offset='1'>
                  <el-form-item label="代号" :prop="'manufacturerList.' + index + '.manufacturerId'"
                    ...
                    <el-input v-model="item.manufacturerId" placeholder="请选择厂商名称" @blur='hanldChange' disabled/>
                  </el-form-item>
                </el-col>
                <el-col :span="6" :offset='1'>
                  <el-form-item>
                    <el-button type="danger" v-if="form.manufacturerList.length > 1" size="mini" style='margin-top:30px'
                               @click='deleteRules(index)' disabled>
                      删除
                    </el-button>
                  </el-form-item>
                </el-col>
              </el-row>
              <el-button @click="addDomain" size='mini' disabled>新增厂商</el-button>
            </el-form>
          </div>
      ...
      <div class='inquiry_warp'>
        <el-form ref="formContact" :rules="rules" :model="form" label-width="100px">
          <el-form-item label="阶梯标准" prop='ladderPrice'>
            <el-input v-model="form.ladderPrice" placeholder="如有多个 已;隔开 例如:0-50;50-100;"></el-input>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          BiddingTime: [
            {required: false, message: '活动时间不能为空', trigger: 'change'}
          ],
          enclosure:[
    ...
    methods: {
      ...
      BiddingTimeChange() {
        // 招商开始时间
        this.form.startTime = this.BiddingTime[0]
        // 招商结束时间
        this.form.endTime = this.BiddingTime[1]
      },
      // 添加询报价方设置，手动新增
      addDomain() {
                  ...
                  this.$refs['manufacturerList'].validate((valid) => {
                    if (valid) {
                      ...
                      const params = {
                        ...
                        // 询报价轮数
                        temderNumber: this.form.temderNumber,
                        // 投标开始时间
                      ...
                      axios.post(config.api, '/offermanagement/UpdateOfferManagement.do', params).then((res) => {
                        if (res.status == 0) {
                          this.$alert('保存成功', {

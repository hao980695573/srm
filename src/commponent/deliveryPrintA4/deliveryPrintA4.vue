        ...
        <div class='tab_company_out'>
          <div class='title_warp'>
            ...
            <div class='title_top'>
              ...
              <div class='title_top_right'>
                <div>
                  <span>单号 ：</span><span>{{tableData.deliveryNumber}}</span>
                </div>
                <div>
                  <span>送货日期 ：</span><span>{{tableData.deliveryDate | filterDate}}</span>
                </div>
                <div>
            ...
            <div class='title_center'>发货单</div>
            <div class='title_bottom'>
              <div class='title_bottom_left'>
                <div>
                  <span>供应商名称: </span><span>{{companyName}}</span>
                </div>
                <div>
            ...
            <div class='PurchaseWrap'>
              <div>
                <div class='kehubianma'>客户编码:{{tableData.erpno}}</div>
          ...
          <div class="table_warp">
            <table cellpadding='0' cellspacing='0'>
              ...
              <tr>
                <td rowspan="3">备注</td>
                <td colspan='8' style='text-align: left'>&nbsp1: 以上货品已收妥,核对无误</td>
              </tr>
              <tr>
                <td colspan='8' style='text-align: left'>&nbsp2: 如有数量及质量问题请在三天内反馈到我司,如无反馈则我司默认数量及质量无误,不再接受退货</td>
              </tr>
              <tr>
                <td colspan='8' style='text-align: left'>&nbsp3: 如发生纠纷,任何一方均可向东莞市第二人民法院起诉</td>
              </tr>
            </table>
            <!--尾页-->
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        axios.post(config.api, '/delivery/getDeliveryInfo.do', params).then((res) => {
          console.log(res);
          if (res.status == 0) {
            this.tableData = res.data
            if (res.data.deliveryProductList.length > 0) {
      ...
      printPdf() {
        LODOP = getLodop();
        var strStyleCSS2 = "<link href='../../../static/styles/ww.css' type='text/css' rel='stylesheet'>";
        ...
        LODOP.ADD_PRINT_HTM("0cm", "30mm", "RightMargin:0cm", "BottomMargin:0cm", strFormHtml);
        LODOP.SET_PRINT_MODE("PRINT_PAGE_PERCENT", "Auto-Width");
        LODOP.PREVIEW()
      }
    },
    computed: {
      ...mapState(['uuid', 'manufacturerAddress', 'enterprisePhone', 'fax', 'Contacts', 'phone', 'supplierBillType', 'companyName']),
      // 打印分页
      pages() {
        // 求当前数据能打印的页数

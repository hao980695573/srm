<template>
  <div class='yulan_warp' v-show='pages>0'>
    ...
    <!--打印按钮-->
    <div class='printBth'>
      <el-button type="primary" size='mini' @click='labelprintPdf'>打印</el-button>
    </div>
  </div>
        ...
        axios.post(config.api, '/barcode/updateBarcodeStatus.do', params).then((res) => {
          if (res.status == 0) {
            ...
            setTimeout(function () {
              ...
              var labelstrFormHtml = "<head>" + strStyleCSS2 + "</head><body>" + document.getElementById("labelPrinting").innerHTML + "</body>";

              LODOP.ADD_PRINT_HTM("2.5mm", "2.5mm", "RightMargin:5cm", "1150", labelstrFormHtml);

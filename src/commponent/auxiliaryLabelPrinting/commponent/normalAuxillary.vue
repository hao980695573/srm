  ...
  export default {
    ...
    data(){
      ...
      return{
        ...
        rules: {
          deliveryNumber: [
            { required: true, message: '请输入送货单号', trigger: ['blur','change']},
          ],
          deliveryCount:[
    ...
    methods:{
      ...
      getDeliveryNote(){
        ...
        axios.post(config.api, '/delivery/getDeliveryList.do', params).then((res) => {
          if (res.status == 0) {
            ...
            let deliveryNumber=[]
            res.data.map((item)=>{
              deliveryNumber.push({value:item.deliveryNumber,uuid:item.uuid,erpno:item.erpno})
            })
            this.restaurants=deliveryNumber
          } else {
            this.$alert(res.msg, {
      ...
      handleSelect(data){
        // 选中的送货单号
        this.selcetDeliveryNumber=data.value
        const params={
          deliveryNumber:data.value,
          uuid:data.uuid
        }
        ...
        // 获取物料编码接口
        axios.post(config.api, '/barcode/getDeliveryInfo.do', params).then((res) => {
          if (res.status == 0) {
            let Arr=[]
            res.data.deliveryProductList.map((item)=>{
              Arr.push({value:item.productName,dproductUuid:item.uuid,quantity:item.quantity,bpCount:item.bpCount})
            })
      ...
      materielSelect(item){
        ...
        // 查找有多少正常数量和备品数量
        this.materielList.map((it)=>{
          if(it.dproductUuid==item.dproductUuid){
            // 可打印数量
            console.log(item.quantity);
            this.form.printableQuantity=item.quantity
          }
        })
      },
      // 填写了最小包装量
      ...
      getBarcodeList(data){
        ...
        axios.post(config.api, '/barcode/getAuxiliaryDetailList.do', params).then((res) => {
          if (res.status == 0) {
            ...
            this.form.mpCount=''
            this.form.zhengCount=''
            this.form.beiCount=''
            this.form.printableQuantity=''
          }else {
            this.$alert(res.msg, {
              confirmButtonText: '确定',
              center: true

  ...
  <div>
    ...
    >
      ...
      <el-table-column
        ...
        width="150">
        <template slot-scope="scope">
          <el-input v-model="scope.row.invoiceNumber" size='mini' placeholder="请输入内容"></el-input>
        </template>
      </el-table-column>
      <el-table-column
        label="创建日期"
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button
            type="text"
            size="mini" @click='saveExamine(scope.row)'>
            保存
          </el-button>
  ...
  export default {
    ...
    methods:{
      ...
      handleCurrentChange(data){
        ...
        axios.post(config.api, '/aStatement/getFillInvoiceInformationList.do', params).then((res) => {
          console.log(res);
          if (res.status == 0) {
            this.form=res.data
            // 分页
      ...
      // 保存
      saveExamine(scope){
        axios.post(config.api, '/aStatement/SaveInvoice.do', scope).then((res) => {
          if (res.status == 0) {
            this.$alert(res.msg, {

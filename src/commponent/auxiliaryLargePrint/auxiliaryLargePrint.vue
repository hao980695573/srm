<template>
  <div class='yulan_warp'>
    <div id="largePrinting">
      <div v-for="(page,index) in barCodeList" :key="index" class='a'>
      ...
      largePrintPdf() {
        ...
        axios.post(config.api, '/barcode/updateBarcodeStatus.do', params).then((res) => {
          if (res.status == 0) {
            ...
            setTimeout(function () {
              LODOP = getLodop();
              var strStyleCSS2 = "<link href='../../../../static/styles/LargeSizePrint.css' type='text/css' rel='stylesheet'>";
              var labelstrFormHtml = "<head>" + strStyleCSS2 + "</head><body>" + document.getElementById("largePrinting").innerHTML + "</body>";

              LODOP.ADD_PRINT_HTM("2.5mm", "3mm", "RightMargin:5cm", "1100", labelstrFormHtml);
